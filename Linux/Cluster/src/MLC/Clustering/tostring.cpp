...
const char * ToString(char * dest, double x, int32_t decimal)
{
	...
	if(y > 1E13 || isnan(x)) {	//	Do not show 1.2E121 with 122 characters ! Keep 1E13, as actual time in ms is greater than 1E12
		...
		sprintf(dest, GFormat[decimal + 1], x) ;
	}
	else {
		sprintf(dest, FFormat[decimal], x) ;
	}
	return(dest) ;
}
...
double	ReadDuration(const wchar_t * TimeString)
{
	...
	wcscpy(StringC, TimeString) ;	//	do not destroy DateString !

	wchar_t * pContext = nullptr ;
    
    //const wchar_t * INFlabel("INF");
    
	StrPtr = wcstok(StringC, Separator, & pContext) ;
	if(StrPtr == nullptr) {
		Hour = 0 ;
	}
	else if(wcscmp(StrPtr, L"INF") == 0) {
		return(cPlusInf) ;
	}
	else {
		Hour = boost::lexical_cast<long>(StrPtr) ;
	}

	StrPtr = wcstok(nullptr, Separator, & pContext) ;
	if(StrPtr == nullptr) {
		Minute = 0 ;
	}
	else {
		Minute = boost::lexical_cast<long>(StrPtr) ;
	}
	StrPtr = wcstok(nullptr, Separator, & pContext) ;
	if(StrPtr == nullptr) {
		Second = 0 ;
		...
		Second = boost::lexical_cast<long>(StrPtr) ;
	}
	StrPtr = wcstok(nullptr, Separator, & pContext) ;
	if(StrPtr == nullptr) {
		Milli = 0 ;
		...
		Milli = boost::lexical_cast<long>(StrPtr) % 1000 ;
	}
