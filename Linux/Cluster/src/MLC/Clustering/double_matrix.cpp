...
{
    //std::wstring tmps(p_FullFileName);
    char namefile[256];
    std::wcstombs(namefile, p_FullFileName, 256);
...
bool double_matrix::Save(const wchar_t * p_FullFileName) const
{
    char namefile[256];
    std::wcstombs(namefile, p_FullFileName, 256);
	std::ofstream file(namefile, std::ofstream::binary) ;
	if(file.fail()) {
		return true ;	//	error
	}
...
OperatorStar(size_t p_Id, matrix_context * ctx)
{
	...
	while(true) {
		...
		//size_t rmax = rmin + max_(1ull, maxComputations / (ctx->cmax - ctx->cmin));		//	search for
        //uint64_t tempmaxmin = ctx->cmax - ctx->cmin;
        size_t tempmaxmin = ctx->cmax - ctx->cmin;
        size_t maxmax = 1;
        size_t rmax = rmin + max_(maxmax, maxComputations / tempmaxmin);
            //	search for
            //rmax giving us at least 1000 computations
...
OperatorPLus(size_t p_Id, matrix_context * ctx)
{
	...
	while (true) {
		...
		size_t rmin = ctx->r;			//	get the first row to compute
		//size_t rmax = rmin + max_(1ull, maxComputations / (ctx->cmax - ctx->cmin));
        //uint64_t tempmaxmin = ctx->cmax - ctx->cmin;
        size_t tempmaxmin = ctx->cmax - ctx->cmin;
        size_t maxmax = 1;
        size_t rmax = rmin + max_(maxmax, maxComputations / tempmaxmin);
        //size_t rmax = rmin + max_(1ull, maxComputations / tempmaxmin);
        //	search for rmax giving us at least 100000 computations
		rmax = min_(rmax, ctx->rmax);
		ctx->r = rmax;					//	update the next row to compute in the context
