...
namespace Sztd {
	...
	template<class T>
	int MemmoveToBuffer(const std::vector<T> & vectorarg, char * buffer)	//	memmove version
	{
		char * Dest = buffer ;
		int VectorSize = vectorarg.size() ;
		*(int *)Dest = VectorSize ;
		Dest += sizeof(VectorSize) ;

		VectorSize *= sizeof(T) ;
		//memmove(Dest, vector.MyPtr(), VectorSize) ;
                memmove(Dest, vectorarg, VectorSize) ;
		Dest += VectorSize ;
	...
	template<class T>
	int MemmoveFromBuffer(std::vector<T> & vectorarg, const char * buffer)	//	memmove version
	{
		const char * Srce = buffer ;
		
		int VectorSize = *(int *)Srce ;
		vectorarg.resize(VectorSize) ;
		Srce += sizeof(VectorSize) ;

		VectorSize *= sizeof(T) ;
		//memmove(vectorarg.MyPtr(), Srce, VectorSize) ;
                memmove(vectorarg, Srce, VectorSize) ;
		Srce += VectorSize ;
