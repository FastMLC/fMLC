...
enum TextFileTypeEnum {			//		BOM
	...
	SCSU = 6,						// 	0E FE FF
	UTF_7 = 7,						// 	2B 2F 76 et l'une des séquences d'octets suivantes: [ 38 | 39 | 2B | 2F ]
...
{
	int32_t LoadFastaFile(vector<TNFieldBase *> & p_Sequences, const wchar_t * p_FilePath, bool p_IsDna, uint32_t p_NamePos);
	int32_t LoadFastaFileUtf8(vector<TNFieldBase *> & p_Sequences, const wchar_t * p_FilePath, bool p_IsDna, uint32_t p_NamePos);
	...
	{
	public:
		OnSequenceSizeDescending_int32(const vector<TNFieldBase *> & p_Sequences)			:m_Sequences(p_Sequences)		{		}
	...
	protected:
		const vector<TNFieldBase *> & m_Sequences;
	};
}
